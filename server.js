...
function getRoom(player){
    return player?.roomId ? rooms.get(player.roomId)||null : null;
}
...
function removeFromMatchmaking(player){
    ...
    while((index=matchmakingQueue.indexOf(player))!==-1){
...
function tryMatchPlayers(){
    while(matchmakingQueue.length>=2){
        ...
        if(!isConnected(player1)||!isConnected(player2))continue;
        if(player1.roomId||player2.roomId)continue;

        const room=createRoom("مباراة أونلاين",player1);
...
function validateFleet(grid,ships){
    if(!Array.isArray(ships)||ships.length!==5){
        return {valid:false,message:"يجب أن يحتوي الأسطول على 5 سفن"};
    }

    const expected=[4,3,3,2,2];
    const used=new Set();
    ...
    for(const size of expected){
        const index=ships.findIndex(
            (s,i)=>s &&
                   Number(s.size)===size &&
                   !normalizedShips.includes(s)
        );

        if(index===-1){
            return {
                valid:false,
                message:"ترتيب السفن غير صحيح"
            };
        }

        normalizedShips.push(ships[index]);
    }

    for(const ship of normalizedShips){
        if(!Array.isArray(ship.cells)||ship.cells.length!==Number(ship.size)){
            return {
                valid:false,
                message:"عدد خلايا إحدى السفن غير صحيح"
            };
        }

        const cells=ship.cells.map(c=>[
            Number(c[0]),
            Number(c[1])
        ]);
        ...
        for(const [r,c] of cells){
            if(!validCell(r,c)){
                return {valid:false,message:"موقع سفينة غير صحيح"};
            }

            const k=key(r,c);

            if(unique.has(k)||used.has(k)){
                return {valid:false,message:"السفن متداخلة"};
            }
            ...
            if(grid[r][c]!=="ship"){
                return {
                    valid:false,
                    message:"بيانات السفن لا تطابق الخريطة"
                };
            }
        }
        ...
        if(!sameRow&&!sameCol){
            return {
                valid:false,
                message:"السفن يجب أن تكون مستقيمة"
            };
        }
        ...
        if(sameRow){
            ...
            for(let i=1;i<sorted.length;i++){
                if(sorted[i]!==sorted[i-1]+1){
                    return {
                        valid:false,
                        message:"خلايا السفينة يجب أن تكون متجاورة"
                    };
                }
            }
        }else{
        ...
        cells.forEach(([r,c])=>used.add(key(r,c)));
    ...
    if(shipCount!==14){
        return {
            valid:false,
            message:"يجب أن يحتوي الأسطول على 14 خانة"
        };
    }
    ...
    if(used.size!==14){
        return {
            valid:false,
            message:"بيانات السفن غير صحيحة"
        };
    }
...
function generateRandomFleet(){
    ...
    throw new Error("تعذر إنشاء أسطول");
}

function setPlayerFleet(player,grid,ships){
    player.grid=normalizeGrid(grid);

    player.ships=ships.map((ship,index)=>({
        id:index,
        size:Number(ship.size),
        cells:ship.cells.map(c=>[Number(c[0]),Number(c[1])]),
        hits:new Set()
    }));

    player.shipCells=new Map();

    player.ships.forEach(ship=>{
        ship.cells.forEach(([r,c])=>{
            player.shipCells.set(key(r,c),ship.id);
        });
    });
}
...
function clearPreparationTimer(room){
    if(room?.preparationTimer){
        clearInterval(room.preparationTimer);
...
function startBattlePreparation(room){
    ...
    room.preparationEndsAt=
        Date.now()+PREPARATION_TIME*1000;
    ...
    room.preparationTimer=setInterval(()=>{
        const remaining=Math.max(
            0,
            Math.ceil(
                (room.preparationEndsAt-Date.now())/1000
            )
        );
...
function updateBattleGrid(player,message){
    ...
    const grid=normalizeGrid(message.grid);
    const validation=validateFleet(grid,message.ships);
...
function handleBattleReady(player,message){
    ...
    if(!room.preparation){
        send(player.ws,{
            type:"error",
            message:"مرحلة ترتيب السفن غير فعالة"
        });
        return;
    }

    updateBattleGrid(player,message);
    ...
    if(room.players.every(p=>p.battleReady)){
        clearPreparationTimer(room);
        startBattle(room);
    }
}
...
function getAttackCells(row,col,weapon){
    if(weapon==="normal"){
        return [{row,col}];
    }
...
function handleAttack(player,message){
    ...
    const weapon=message.weapon==="bomb"?"bomb":"normal";
    ...
    if(weapon==="bomb" && player.weapons.bomb<=0){
        send(player.ws,{
            type:"error",
            message:"لا توجد قنابل متبقية"
        });
        return;
    }

    const cells=getAttackCells(row,col,weapon);

    /*
       لا نسمح للقنبلة بضرب خانات سبق استهدافها.
    */
    if(cells.some(c=>player.attackedCells.has(key(c.row,c.col)))){
        send(player.ws,{
            type:"error",
            message:"منطقة الهجوم تحتوي على خانة تم استهدافها من قبل"
        });
        return;
    }
    ...
    for(const cell of cells){
        ...
        const shipId=opponent.shipCells.get(k);
        const hit=shipId!==undefined;

        if(hit){
            anyHit=true;

            const ship=opponent.ships[shipId];

            ship.hits.add(k);

            opponent.grid[cell.row][cell.col]="hit";

            if(ship.hits.size===ship.cells.length){
                sunkShips.add(shipId);
            }
        }else{
            opponent.grid[cell.row][cell.col]="miss";
        }
    ...
    const attackResult={
        ...
        hit:anyHit,
        cells:results,
        nextPlayerId:anyHit?player.id:opponent.id
    };
    ...
    /*
       ...
       سفينة 2 = لا مكافأة.
    */
    for(const shipId of sunkShips){
        const ship=opponent.ships[shipId];

        if(ship.size===4||ship.size===3){
            player.weapons.bomb++;
...
function handleMessage(player,message){
    ...
    switch(message.type){
        ...
        case "cancelMatch":
            removeFromMatchmaking(player);
            send(player.ws,{type:"matchCancelled"});
            break;
